...
Defines the home dashboard's "Live now" section: when it appears, which matches it lists (those currently in progress for the selected group), what each row shows (teams, current score, a live indicator), the 👁 affordance to view group members' bets for that match, and how the section refreshes in step with the existing live-score poller.
